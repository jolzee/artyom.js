...
export default class Artyom {
  ...
  constructor() {
    ...
    if (window.hasOwnProperty("webkitSpeechRecognition")) {
      this.ArtyomWebkitSpeechRecognition = new (<any>(
        window
  ...
  execute(voz): MatchedCommand {
    ...
    if (_this.ArtyomProperties.name) {
      if (voz.indexOf(_this.ArtyomProperties.name) != 0) {
        _this.debug(
          `Artyom requires with a name "${_this.ArtyomProperties.name}" but the name wasn't spoken.`,
          "warn"
        );
        return;
      }
      ...
      for (let c = 0; c < opciones.length; c++) {
        ...
        if (opcion instanceof RegExp) {
          ...
          if (opcion.indexOf("*") != -1) {
            ...
            if (grupo.length > 2) {
              console.warn(
                "Artyom found a smart command with " +
                  (grupo.length - 1) +
    ...
    for (let i = 0; i < _this.ArtyomCommands.length; i++) {
      ...
      for (let c = 0; c < opciones.length; c++) {
        ...
        if (voz === opcion) {
          ...
          _this.debug(
            ">> MATCHED OPTION CHANGING ALL TO LOWERCASE " +
              opcion +
              " AGAINST " +
              voz +
              " WITH INDEX " +
    ...
    for (let i = 0; i < _this.ArtyomCommands.length; i++) {
      ...
      for (let c = 0; c < opciones.length; c++) {
        ...
        if (voz.indexOf(opcion) >= 0) {
          ...
          _this.debug(
            ">> MATCHED INDEX OPTION CHANGING ALL TO LOWERCASE " +
              opcion +
              " AGAINST " +
              voz +
              " WITH INDEX " +
      ...
      for (let i = 0; i < _this.ArtyomCommands.length; i++) {
        ...
        for (let c = 0; c < opciones.length; c++) {
          ...
          if (_this.soundex(voz) == _this.soundex(opcion)) {
            _this.debug(
              `>> Matched Soundex command '${opcion}' AGAINST '${voz}' with index ${c}`,
              "info"
            );
    ...
    _this.debug(
      `Event reached : ${_this.ArtyomGlobalEvents.NOT_COMMAND_MATCHED}`
    );
  ...
  hey(resolve: Function, reject: Function) {
    ...
    this.ArtyomWebkitSpeechRecognition.onstart = () => {
      _this.debug(
        "Event reached : " + _this.ArtyomGlobalEvents.COMMAND_RECOGNITION_START
      );
      ...
      if (event.error == "not-allowed") {
        ...
        if (event.timeStamp - start_timestamp < 100) {
          _this.triggerEvent(_this.ArtyomGlobalEvents.ERROR, {
            code: "info-blocked",
            message:
              "Artyom needs the permission of the microphone, is blocked.",
          });
        } else {
          _this.triggerEvent(_this.ArtyomGlobalEvents.ERROR, {
            code: "info-denied",
      ...
      onResultProcessor = (event) => {
        ...
        for (let i = event.resultIndex; i < cantidadResultados; ++i) {
          ...
          if (event.results[i].isFinal) {
            ...
            if (
              typeof _this.ArtyomProperties.helpers
                .redirectRecognizedTextOutput === "function"
            ) {
              _this.ArtyomProperties.helpers.redirectRecognizedTextOutput(
                identificated,
                true
              );
            }
            ...
            if (comando && _this.ArtyomProperties.recognizing == true) {
              _this.debug(
                "<< Executing Matching Recognition in normal mode >>",
                "info"
              );
              _this.ArtyomWebkitSpeechRecognition.stop();
              _this.ArtyomProperties.recognizing = false;
              ...
              if (comando.wildcard) {
                comando.instruction.action(
                  comando.index,
                  comando.wildcard.item,
                  comando.wildcard.full
                );
            ...
            if (typeof _this.ArtyomProperties.executionKeyword === "string") {
              if (
                identificated.indexOf(
                  _this.ArtyomProperties.executionKeyword
                ) != -1
              ) {
                let comando = _this.execute(
                  identificated
                ...
                if (comando && _this.ArtyomProperties.recognizing == true) {
                  _this.debug(
                    "<< Executing command ordered by ExecutionKeyword >>",
                    "info"
                  );
                  _this.ArtyomWebkitSpeechRecognition.stop();
                  _this.ArtyomProperties.recognizing = false;
      ...
      onResultProcessor = (event) => {
        ...
        for (let i = event.resultIndex; i < cantidadResultados; ++i) {
          ...
          if (!event.results[i].isFinal) {
            ...
            if (comando && _this.ArtyomProperties.recognizing == true) {
              _this.debug(
                "<< Executing Matching Recognition in quick mode >>",
                "info"
              );
              _this.ArtyomWebkitSpeechRecognition.stop();
              _this.ArtyomProperties.recognizing = false;
    ...
    if (_this.ArtyomProperties.mode == "remote") {
      onResultProcessor = (event) => {
        ...
        if (
          typeof _this.ArtyomProperties.helpers.remoteProcessorHandler !==
          "function"
        ) {
          return _this.debug(
            "The remoteProcessorService is undefined.",
            "warn"
          );
        }
    ...
    _this.ArtyomWebkitSpeechRecognition.onresult = (event) => {
      if (_this.ArtyomProperties.obeying) {
        ...
        if (
          interim.indexOf(_this.ArtyomProperties.obeyKeyword) > -1 ||
    ...
    if (_this.ArtyomProperties.recognizing) {
      _this.ArtyomWebkitSpeechRecognition.stop();
      _this.debug(
        "Event reached : " + _this.ArtyomGlobalEvents.COMMAND_RECOGNITION_END
      );
      _this.triggerEvent(_this.ArtyomGlobalEvents.COMMAND_RECOGNITION_END);
    } else {
      try {
        ...
        _this.triggerEvent(_this.ArtyomGlobalEvents.ERROR, {
          ...
          message:
            "A webkitSpeechRecognition instance has been started while there's already running. Is recommendable to restart the Browser",
        });
      }
    }
  }
  ...
  initialize(config: ArtyomProperties): Promise<boolean> {
    ...
    if (typeof config !== "object") {
      return Promise.reject(
        "You must give the configuration for start artyom properly."
      );
    }
  ...
  getVoice(languageCode: string) {
    ...
    if (!voiceIdentifiersArray) {
      console.warn(
        `The provided language ${languageCode} isn't available, using English Great britain as default`
      );
    ...
    for (let i = 0; i < voicesLength; i++) {
      let foundVoice = voices.filter((voice) => {
        return (
          voice.name == voiceIdentifiersArray[i] ||
    ...
    if (typeof foundCommand === "object") {
      if (foundCommand.instruction) {
        if (foundCommand.instruction.smart) {
          _this.debug(
            "Smart command matches with simulation, executing",
            "info"
          );
  ...
  talk(
    ...
    if (actualChunk == 1) {
      msg.addEventListener("start", function () {
        ...
        _this.debug(
          "Event reached : " + _this.ArtyomGlobalEvents.SPEECH_SYNTHESIS_START
        );
    ...
    if (actualChunk >= totalChunks) {
      msg.addEventListener("end", function () {
        ...
        _this.debug(
          "Event reached : " + _this.ArtyomGlobalEvents.SPEECH_SYNTHESIS_END
        );
    ...
    this.debug(
      actualChunk + " text chunk processed succesfully out of " + totalChunks
    );
      ...
      if (message.length > artyom_say_max_chunk_length) {
        ...
        naturalReading.forEach((chunk, index) => {
          ...
          if (chunk.length > artyom_say_max_chunk_length) {
            ...
            let temp_processed = _this.splitStringByChunks(
              chunk,
              artyom_say_max_chunk_length
            );
